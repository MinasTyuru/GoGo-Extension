...
public class GoGoExtension extends org.nlogo.api.DefaultClassManager {
    ...
    public void load(org.nlogo.api.PrimitiveManager primManager) {
        ...
        try {
            ...
            if (osname.equals("Mac OS X")) {
                ...
                libdir = libdir + systype;
            }
            org.nlogo.api.JavaLibraryPath.add(new java.io.File(libdir));

        } catch (Exception ex) {
            System.err.println("Cannot add GoGo libraries to java.library.path: " + ex.getMessage());
        }
    ...
    public void runOnce(org.nlogo.api.ExtensionManager extensionManager) throws ExtensionException {
        if (System.getProperty("os.name").startsWith("Windows")) {
            final String baseDirPath = new File(this.getClass().getProtectionDomain().getCodeSource().getLocation().getFile()).getParent();
            final String fileSep = System.getProperty("file.separator");
            verifyDriverValidity(baseDirPath, fileSep, extensionManager);
        }
    }
    ...
    private boolean obtainPermissionToInstall(org.nlogo.api.ExtensionManager extensionManager) {
        ...
        //System.err.println("Starting to obtain permission.");

        try {

            final GUIWorkspace gw = ((GUIWorkspace) ((org.nlogo.workspace.ExtensionManager) extensionManager).workspace());

            int result = gw.waitForResult(new ReporterRunnable<java.lang.Integer>() {
                public java.lang.Integer run() {
                    return OptionDialog.show(gw.getFrame(), "User Message", WINDOWS_PROMPT_MESSAGE,
